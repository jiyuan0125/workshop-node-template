...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		fn gen_unique_id() -> ([u8; 16], Color) {
			...
			// Create randomness payload. Multiple collectibles can be generated in the retaining uniqueness.
			let unique_payload = (
				random,
		...
		pub fn mint(
			...
			Self::deposit_event(Event::<T>::CollectibleCreated {
		...
		// and internal function for purchasing a collectible
		pub fn do_buy_collectible(
			...
			let from = collectible.owner.clone();
			ensure!(from != to, Error::<T>::TransferToSelf);
	...
	impl<T: Config> Pallet<T> {
		...
		///
		/// The autual collectible crateion is done in the `mint()` function.
		#[pallet::weight(0)]
		...
		/// Any account that holds a collectible can send ti to another account.
		/// Transfer resets the price of the collectible, marking it not fot sale.
		#[pallet::weight(0)]
		pub fn transfer(
			origin: OriginFor<T>,
			to: T::AccountId,
